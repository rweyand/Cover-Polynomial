/* 
 * File:   main.c
 * Author: Reinhard Weyand
 *
 * Implementation of the fast evaluation algorithm for the coefficients of the cover polynomial
 * as presented in "Computing The Tutte Polynomial In Vertex-Exponential Time" by Björklund et. al. (Appendix p.3-4).
 */

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <gmp.h>
#include <unistd.h>
#include <limits.h>

#include "remaindering.h"
#include "tools.h"
#include "matrix.h"
#include "poly.h"
#include "typedefs.h"
scalar* P_U_I;
scalar* C_U_J; //C-polynomial to the power of J
scalar* PC_IJ; //(P^i)*(C*j)

/*
 * Compute the number of directed walks of length l in G[S] by polynomial exponentiation
 * S is the subset of vertices under consideration
 * s and t are the start- and end-vertex for the walk
 * l is the length of the walks to count, i.e. the exponent for matrix exponentiation
 */


void freeAll() { //cascade for cleaning up Memory   
    int i,j;
    for (i = 0; i < n + 1; i++)
        for (j = 0; j < n + 1; j++)
            free(VALS(i, j));
    free(vals);
    free(PC_IJ);
    free(P_U_I);
    free(C_U_J);
    free(modules);
    free(coefficients);
    
}

void init() {
    P_U_I = (scalar*) malloc(((n + 1)*(n + 1) + 1) * sizeof (scalar));
    P_U_I[0] = 0;
    C_U_J = (scalar*) malloc(((n + 1)*(n + 1) + 1) * sizeof (scalar));
    C_U_J[0] = 0;
    PC_IJ = (scalar*) malloc(((n + 1)*(n + 1)*(n + 1) + 1) * sizeof (scalar));
    PC_IJ[0] = 0;
}
scalar w(set S, int s, int t, int l, int m) {
    scalar module = modules[m];
    int i, j; //positions in submatrix
    int size = getNumOfNodes(S);
    i = getPos(s, S);
    j = getPos(t, S);
    if (!(ELEM(s, S) && ELEM(t, S)))
        return 0;
    if ((s == t) && (l == 0))
        return 1;
    scalar* subMatrix = (scalar*) malloc(size * size * sizeof (scalar));
    scalar* result = (scalar*) malloc(size * size * sizeof (scalar));
    getSubMatrix(S, subMatrix);
    matrixPow(subMatrix, l, result, size, module);
    scalar res = matrix(result, i, j, size);
    //  printf("freeing matrices of size %d x %d \n computed result: %d \n",size,size,res);	
    free(result);
    //	printf("freeing submatrix \n");
    free(subMatrix);
    return res;
}

/*
 * compute the polynomial
 
 
 */
void computeC_X(int m) { //compute number of spanning directed cycles for X according to björklund et.al. by fast moebius inversi
    scalar module = modules[m];
    int S, j, k, s;
    c = (scalar*) malloc(pow2 * sizeof (scalar));
    for (S = 0; S < pow2; S++)
        c[S] = 0;
    scalar* b_flat = (scalar*) malloc(pow2 * (n + 1) * sizeof (scalar));
    int size;
    for (s = 0; s < n; s++)
        for (k = 0; k <= n; k++) {
            //	printf("k	=	%d  \n",k);
            for (S = 0; S < pow2; S++) {
                size = popCnt(S);
                B(S, 0) = w(S, s, s, k, m);
                for (j = 1; j <= n; j++) {
                    if ((1 << (j - 1)) & S) {
                        B(S, j) = MYMOD(B(S, j - 1) - B(S^(1 << (j - 1)), j - 1), module);
                    } else {
                        B(S, j) = B(S, j - 1);
                    }
                }
                if (k == size) {
                    c[S] = MYMOD(c[S] + B(S, n), module);

                }
            }
        }
    for (S = 1; S < pow2; S++) {
        if (n >= 10) {
            scalar intInverse = getIntInverse(popCnt(S), module);
            c[S] = MYMOD(c[S] * intInverse, module);
        } else
            c[S] /= popCnt(S);
    }
    free(b_flat);
}

void computeP_X(int m) { //compute number of spanning directed paths for X according to björklund et.al. by fast moebius inversion	
    scalar module = modules[m];
    int k, j, s, t;
    set S;
    int size;
    p = (scalar*) malloc(pow2 * sizeof (scalar));
    scalar* a_flat = (scalar*) malloc(pow2 * (n + 1) * sizeof (scalar));
    for (S = 0; S < pow2; S++)
        p[S] = 0;
    for (s = 0; s < n; s++) {
        for (t = 0; t < n; t++) {
            for (k = 0; k <= n; k++) {
                for (S = 0; S < pow2; S++) {
                    size = popCnt(S);
                    A(S, 0) = w(S, s, t, k, m);
                    for (j = 1; j <= n; j++) {
                        if ((1 << (j - 1)) & S)
                            A(S, j) = MYMOD(A(S, j - 1) - A(S^(1 << (j - 1)), j - 1), module); //f[j](X) = - f[j-1](X\{j}) + f[j-	 
                        else
                            A(S, j) = A(S, j - 1); //f[j](X) = f[j-1](X)
                    }
                    if (k == (size - 1)) {
                        p[S] = MYMOD(p[S] + A(S, n), module);
                    }
                }

            }
        }
    }
    free(a_flat);
}

void computeP_U_S(set U, set S, polynom* subsetPolynom, int m) {
    scalar module = modules[m];
    int s, t, k, i, sign;
    int size = popCnt(U^S);
    int size_S = popCnt(S);
    polynom poly = *subsetPolynom;
    for (i = 0; i < n + 3; i++)
        poly[i] = 0;
    for (s = 0; s < n; s++) {
        for (t = 0; t < n; t++) {
            for (k = 0; k <= size; k++) {
                sign = (k % 2 == 0) ? 1 : -1;
                poly[size_S + k + 1] = MYMOD(poly[size_S + k + 1] + MYMOD(sign * (w(S, s, t, size_S + k - 1, module) * binCoeff(size, k, module)), module), module);
                if (poly[0] < (size_S + k + 1))
                    poly[0] = size_S + k + 1;
            }
        }
    }

}

void computeP_U_Space(int m) {
    scalar module = modules[m];
    int i, size;
    set U;
    set* subsets;
    polynom subsetPolynom;
    P_U = (scalar**) malloc(pow2 * sizeof (scalar*));
    for (U = 0; U < pow2; U++) {
        P_U[U] = (polynom) malloc((n + 3) * sizeof (scalar));
        for (i = 0; i < n + 3; i++)
            P_U[U][i] = 0;
        size = popCnt(U);
        subsets = (set*) malloc(sizeof (set) * myPow(size));
        getSubsets(subsets, U, size);
        for (i = 0; i < myPow(size); i++) {
            subsetPolynom = (scalar*) malloc((n + 3) * sizeof (scalar));
            computeP_U_S(U, subsets[i], &subsetPolynom, module);
            polyAdd(P_U[U], subsetPolynom, P_U[U], module);
            free(subsetPolynom);

        }

    }

}

void computeC_U_S(set U, set S, polynom* subsetPolynom, int m) {
    scalar module = modules[m];
    int s, k, i, sign;
    int size = popCnt(U^S);
    int size_S = popCnt(S);
    polynom poly = *subsetPolynom;
    for (i = 0; i < n + 2; i++)
        poly[i] = 0;
    if (S == 0)
        return;
    for (k = 0; k <= size; k++) {
        for (s = 0; s < n; s++) {
            sign = (k % 2 == 0) ? 1 : -1;
            poly[size_S + k + 1] = MYMOD(poly[size_S + k + 1] + MYMOD(sign * (w(S, s, s, size_S + k, module) * binCoeff(size, k, module)), module), module);
            if (poly[0] < (size_S + k + 1))
                poly[0] = size_S + k + 1;
        }
    }
}

void computeC_U_Space(int m) {
    scalar module = modules[m];
    int i, size;
    set U;
    set* subsets;
    polynom subsetPolynom;
    C_U = (polynom*) malloc(pow2 * sizeof (polynom));
    for (U = 0; U < pow2; U++) {
        C_U[U] = (polynom) malloc((n + 2) * sizeof (scalar));
        for (i = 0; i < n + 2; i++)
            C_U[U][i] = 0;
        size = popCnt(U);
        subsets = (set*) malloc(sizeof (set) * myPow(size));
        getSubsets(subsets, U, size);
        for (i = 0; i < myPow(size); i++) {
            subsetPolynom = (polynom) malloc((n + 2) * sizeof (scalar));
            computeC_U_S(U, subsets[i], &subsetPolynom, module);
            polyAdd(C_U[U], subsetPolynom, C_U[U], module);
            free(subsetPolynom);

        }
        for (i = 2; i < n + 2; i++) {
            C_U[U][i] /= (i - 1);
        }
    }

}

void computeC_U_moebius(int m) {
    computeC_X(m);
    scalar module = modules[m];
    int S, i, size, j;
    C_U = (scalar**) malloc(pow2 * sizeof (scalar*));
    scalar** c_flat = (scalar**) malloc(pow2 * (n + 1) * sizeof (scalar*));
    for (j = 0; j < (n + 1); j++) {
        for (S = 0; S < pow2; S++) {
            C_POLY(S, j) = (scalar*) malloc((n + 2) * sizeof (scalar));
            C_POLY(S, j)[0] = 0;
        }
    }
    for (S = 0; S < pow2; S++) {
        C_U[S] = (scalar*) malloc((n + 2) * sizeof (scalar));
        size = popCnt(S);
        for (i = 0; i < n + 2; i++)
            C_POLY(S, 0)[i] = 0;
        C_POLY(S, 0)[size + 1] = c[S];
        if (c[S] != 0)
            C_POLY(S, 0)[0] = size + 1;
        for (j = 1; j <= n; j++) {
            if (ELEM(j - 1, S)) {
                polyAdd(C_POLY(S, j - 1), C_POLY(S^(1 << (j - 1)), j - 1), C_POLY(S, j), module);
            } else {
                polyCopy(C_POLY(S, j - 1), C_POLY(S, j));
            }
        }
        for (i = 0; i <= C_POLY(S, n)[0]; i++) {
            C_U[S][i] = C_POLY(S, n)[i];
        }
    }
    free(c);
    for (i = 0; i < (n + 1); i++) {
        for (S = 0; S < pow2; S++) {
            free(C_POLY(S, i));
        }
    }
    free(c_flat);
}

void computeP_U_moebius(int m) {
    computeP_X(m);
    scalar module = modules[m];
    int S, i, size, j;
    P_U = (scalar**) malloc(pow2 * sizeof (scalar*));
    scalar** c_flat = (scalar**) malloc(pow2 * (n + 1) * sizeof (scalar*));
    for (j = 0; j < (n + 1); j++) {
        for (S = 0; S < pow2; S++) {
            C_POLY(S, j) = (scalar*) malloc((n + 3) * sizeof (scalar));
            C_POLY(S, j)[0] = 0;
        }
    }
    for (S = 0; S < pow2; S++) {
        P_U[S] = (scalar*) malloc((n + 3) * sizeof (scalar));
        size = popCnt(S);
        for (i = 0; i < n + 1; i++)
            C_POLY(S, 0)[i] = 0;
        C_POLY(S, 0)[size + 1] = p[S];
        if (p[S] != 0)
            C_POLY(S, 0)[0] = size + 1;
        for (j = 1; j <= n; j++) {
            //		printf("S = %d, j = %d!\n",S,j);
            if (ELEM(j - 1, S)) {
                polyAdd(C_POLY(S, j - 1), C_POLY(S^(1 << (j - 1)), j - 1), C_POLY(S, j), module);
            } else {
                polyCopy(C_POLY(S, j - 1), C_POLY(S, j));

            }
        }
        for (i = 0; i <= C_POLY(S, n)[0]; i++) {
            P_U[S][i] = C_POLY(S, n)[i];
        }
    }
    for (i = 0; i < (n + 1); i++) {
        for (S = 0; S < pow2; S++) {
            free(C_POLY(S, i));
        }
    }
    free(p);
    free(c_flat);
}

void computeC_D(int i, int j, int m) {
    scalar module = modules[m];
    VAL(i, j, m) = 0;
    int S, sign, size;
    for (S = 0; S < pow2; S++) {
        size = popCnt(S^(pow2 - 1));
        sign = size & 1 ? -1 : 1;
        polyPow(P_U[S], i, P_U_I, module);
        polyPow(C_U[S], j, C_U_J, module);
        polyMul(P_U_I, C_U_J, PC_IJ, module);
        VAL(i, j, m) = MYMOD(VAL(i, j, m) + ((PC_IJ[0] < n + 1) ? 0 : MYMOD(sign * PC_IJ[n + 1], module)), module);
    }
}

int main(int argc, char** argv) {
    if (argc < 2) {
        fprintf(stderr, "no file specified\n");
        exit(EXIT_FAILURE);
    }
    int opt = -1;
    char* filename = (char*) malloc(50 * sizeof (char));
    while ((opt = getopt(argc, argv, "f:")) != -1) {
        switch (opt) {
            case 'f':
                strcpy(filename, optarg);
                parseGraph(filename);
                break;
            default:
                fprintf(stderr, "no file specified \n");
        }
    }
    init();
    pow2 = mypow(n);
    modules = getModules(n, m, &numModules);
    int i, j, m;
    vals = (scalar**) malloc((n + 1)*(n + 1) * sizeof (scalar*));
    for (i = 0; i < n + 1; i++)
        for (j = 0; j < n + 1; j++)
            VALS(i, j) = (scalar*) malloc(numModules * sizeof (scalar));
    for (m = 0; m < numModules; m++) {
        computeP_U_moebius(m);
        printf("done for dycles. \n");
        computeC_U_moebius(m);
        printf("donefor paths. \n");
        for (i = 0; i <= n; i++) {
            for (j = 0; j <= n; j++) {
                computeC_D(i, j, m);
            }
        }
    }
    printf("done with main loop \n");
    mpz_t i_j_fac, i_fac, j_fac;
    mpz_init(i_fac);
    mpz_init(j_fac);
    mpz_init(i_j_fac);
    coefficients = (mpz_t*) malloc((n + 1)*(n + 1) * sizeof (mpz_t));
    for (i = 0; i < n + 1; i++)
        for (j = 0; j < n + 1; j++) {
            mpz_fac_ui(i_fac, (unsigned long) i);
            mpz_fac_ui(j_fac, (unsigned long) j);
            mpz_mul(i_j_fac, i_fac, j_fac);
            chineseRemainder(modules, VALS(i, j), numModules, COEFFS(i, j));
            mpz_div(COEFFS(i, j), COEFFS(i, j), i_j_fac);

        }
    printCoefficients();
    free(filename);
    mpz_clears(i_fac, j_fac, i_j_fac, NULL);
    freeAll();
    return (EXIT_SUCCESS);
}
